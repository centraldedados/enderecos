...
cache_dir = './cache'
data_dir = './data'
out_path = data_dir + '/portugal.csv'

base_url = 'http://inspire.ine.pt/AD/atom/downloadservice_en.xml'

# Check if cache dir exists or else create it
if not os.path.exists(cache_dir):
    os.makedirs(cache_dir)

# Check if data dir exists or else create it
  ...
  if nuts_url[0:len(base_pattern)] == base_pattern:
    ...
    for file_link in nuts_page.findall('.//entry/link'):
      ...
      if file_url.endswith(file_pattern):
        concelho_info = file_link.getparent().find('./title').text_content()
        concelho = concelho_info.split('-')[0].strip()[4:].encode('utf8').strip().capitalize()
        concelho_filename_utf8 = concelho.replace(" ", "_")
        concelho_filename = unidecode.unidecode(unicode(concelho_filename_utf8, "utf-8")).lower()
        # Append to concelhos dictionary
        filename = str(file_url.split('/')[-1:][0])
        concelhos[filename] = concelho_filename
        #print concelhos.items()
...
for cached_file in [ fi for fi in sorted(os.listdir(cache_dir)) if fi.endswith(".zip")]:
  filename = str(cached_file.split('/')[-1:][0])
  concelho = concelhos.get(filename)

  concelho_csvfile = open(data_dir + '/' + concelho + '.csv', 'w')
  concelho_writer = csv.DictWriter(concelho_csvfile, fieldnames=fieldnames)
  concelho_writer.writeheader()

  print '\nParsing ' + concelho+ ' - ' + cached_file
  postdescriptors = {}
  thoroughfarenames = {}
  ...
  with zipfile.ZipFile(cache_file_path(cached_file), 'r') as zip_file:
    ...
    for ad in zip_page.findall('.//member/address', namespaces={'wfs':'http://schemas.opengis.net/wfs/2.0/wfs.xsd'}):
      ...
      # Write to concelho CSV
      concelho_writer.writerow({'id':ad_id,'street':ad_tf,'house':ad_num,'postcode':ad_pc['pc_postcode'],'city':ad_pc['pc_locality'],'lon':ad_pos[1],'lat':ad_pos[0]})

    
    print 'Parsed ' + str(count) + ' rows'

    concelho_csvfile.close()
